...
async def recognize_image(
    ...
    parsed_result = call_openai_recognize_image(base64_image, file.content_type)

    return parsed_result


class FoodRecognitionResult(BaseModel):
    confidence: float = Field(..., ge=0, le=1, description="表达分类判断的可信程度，0~1 之间小数")
...
def call_openai_recognize_image(base64_image: str, image_type: str) -> str:
    # 定义系统提示词
    prompt = '''
你是一个食品识别助手，请识别图片，并返回符合以下 json schema 的输出：
{"type":"object","properties":{"confidence":{"type":"number","title":"置信度","minimum":0,"maximum":1,"exclusiveMinimum":0,"exclusiveMaximum":1,"description":"表达分类判断的可信程度，0~1 之间小数"},"expiry_days":{"type":"integer","description":"食物的有效期，单位：天","title":"有效期"},"food_name":{"type":"string","title":"食物名","description":"2~5 个字左右的食物名"}},"required":["confidence","expiry_days","food_name"]}
'''
    ...
    return food_recognition_result.model_dump_json()
